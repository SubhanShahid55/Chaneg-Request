...
const publicPaths = ['/', '/login', '/portal/login', '/accept-invite', '/forgot-password', '/reset-password', '/set-password', '/approval'];
...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const finish = async () => {
      ...
      if (token && (pathname === '/' || pathname === '/login' || pathname === '/portal/login')) {
        try {
          ...
          if (active) {
            router.replace(pathname.startsWith('/portal') ? '/portal/login?invalid=true' : '/login?invite=invalid');
          }
        }
        return;
      }
