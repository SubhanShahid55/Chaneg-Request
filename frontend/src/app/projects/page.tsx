...
export default function ProjectsPage() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try { await createProject({ client_id: clientId, name, scope_summary: scope, agreed_budget: budget ? Number(budget) : undefined, timeline_days: timeline ? Number(timeline) : undefined, deliverables: items.filter((item) => item.description.trim()).map((item) => ({ ...item })) }); setShowCreate(false); setName(''); setScope(''); setBudget(''); setTimeline(''); setItems([emptyItem()]); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to create project.'); }
  };

  return <div className="min-h-screen bg-[#f8f9ff]"><Header /><main className="mx-auto max-w-[75rem] px-4 pb-16 pt-24 md:px-6"><div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-[#0b1c30]"><span className="material-symbols-outlined text-[#4f46e5]">folder</span>Projects</h1><p className="mt-1 text-sm text-[#464555]">Track original scope and approved change work by client project.</p></div><button type="button" onClick={() => setShowCreate((open) => !open)} className="rounded-lg bg-[#4f46e5] px-4 py-2 text-sm font-semibold text-white">{showCreate ? 'Close form' : '+ New project'}</button></div>
