...
export default function ResetPasswordPage() {
  ...
  useEffect(() => { void consumeAuthCallback().then((session) => { localStorage.setItem('changeflow_access_token', session.access_token); setReady(true); }).catch((cause) => setError(cause instanceof Error ? cause.message : 'This reset link is invalid or expired.')); }, []);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (password.length < 8 || password !== confirmation) { setError(password.length < 8 ? 'Your password must be at least 8 characters.' : 'The passwords do not match.'); return; } try { await setInvitationPassword(password); router.replace('/dashboard'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to update your password.'); } }
