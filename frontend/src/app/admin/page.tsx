...
export default function AdminPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await inviteAdminUser({ name: name.trim(), email: normalizedEmail, role });
    ...
    <main className="min-h-screen bg-[#f6f8f7] px-5 pb-16 pt-8 text-[#18322b] md:px-10 md:pt-12">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 rounded-2xl border border-[#d6e5de] bg-white p-6 shadow-[0_8px_24px_rgba(28,61,49,0.05)] md:p-7">
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-4 lg:grid-cols-[1fr_1fr_190px_auto] lg:items-end"><Field label="Full name"><input required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Jordan Lee" className="field" /></Field><Field label="Email address"><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" className="field" /></Field><Field label="Role"><select value={role} onChange={(event) => setRole(event.target.value as 'admin' | 'standard')} className="field"><option value="standard">Standard user</option><option value="admin">Administrator</option></select></Field><button type="submit" disabled={submitting} className="rounded-lg bg-[#176b57] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#115642] disabled:cursor-not-allowed disabled:opacity-60">{submitting ? 'Sending invitation...' : 'Send invitation'}</button></form>
        </section>
