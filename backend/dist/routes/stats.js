...
router.get('/summary', async (_req, res) => {
    const cached = await getCached('stats:summary');
    if (cached) {
        res.json(cached);
        return;
    }
 ...
 */
router.get('/weekly-velocity', async (_req, res) => {
 ...
 */
router.get('/weekly-intake', async (_req, res) => {
    const eightWeeksAgo = new Date();
    eightWeeksAgo.setDate(eightWeeksAgo.getDate() - 56);
    ...
    for (const r of requests || []) {
        ...
        if (r.status === 'draft')
            weeks[weekKey].draft++;
        else if (['pending', 'awaiting_approval'].includes(r.status))
            weeks[weekKey].pending++;
...
router.get('/status-breakdown', async (_req, res) => {
    const cached = await getCached('stats:status-breakdown');
    if (cached) {
        res.json(cached);
        return;
    }
