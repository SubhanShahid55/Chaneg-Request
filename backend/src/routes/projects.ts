...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!body.client_id || !name) { res.status(400).json({ error: 'A client and project name are required.' }); return; }
  let deliverables;
  try { deliverables = normalizeDeliverables(body.deliverables); } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Invalid deliverables.' }); return; }
  const { data: project, error } = await supabaseAdmin.from('projects').insert({
    client_id: body.client_id, name, description: body.description || null, scope_summary: body.scope_summary || null,
    agreed_budget: typeof body.agreed_budget === 'number' ? body.agreed_budget : null,
    timeline_days: typeof body.timeline_days === 'number' ? body.timeline_days : null,
  }).select().single();
  if (error || !project) { res.status(500).json({ error: error?.message || 'Unable to create project.' }); return; }
  const { error: deliverableError } = await supabaseAdmin.from('project_deliverables').insert(deliverables.map((item) => ({ ...item, project_id: project.id })));
  if (deliverableError) { await supabaseAdmin.from('projects').delete().eq('id', project.id); res.status(500).json({ error: deliverableError.message }); return; }
